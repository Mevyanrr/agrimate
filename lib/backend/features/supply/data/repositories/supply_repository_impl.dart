...
class SupplyRepositoryImpl implements SupplyRepository {
  ...
  SupplyForecastModel _model(SupplyForecast value) => SupplyForecastModel(
    ...
    harvestEndDate: value.harvestEndDate,
  ...
  Future<Result<List<SupplyForecast>>> _list(
    ...
    try {
      return Success<List<SupplyForecast>>(await load());
    } catch (error) {
      return Failure<List<SupplyForecast>>('$error');
    }
  }
  ...
  Future<Result<SupplyForecast>> create(SupplyForecast supply) async {
    try {
      return Success<SupplyForecast>(await _source.create(_model(supply)));
    } catch (error) {
      return Failure<SupplyForecast>('$error');
    }
  }
  ...
  Future<Result<SupplyForecast>> updateMine(SupplyForecast supply) async {
    try {
      return Success<SupplyForecast>(await _source.updateMine(_model(supply)));
    } catch (error) {
      return Failure<SupplyForecast>('$error');
    }
  }
