...
class DemandRepositoryImpl implements DemandRepository {
  ...
  DemandForecastModel _model(DemandForecast value) => DemandForecastModel(
    ...
    neededStartDate: value.neededStartDate,
    neededEndDate: value.neededEndDate,
  ...
  Future<Result<List<DemandForecast>>> _list(
    ...
    try {
      return Success<List<DemandForecast>>(await load());
    } catch (error) {
      return Failure<List<DemandForecast>>('$error');
    }
  }
  ...
  Future<Result<List<DemandForecast>>> getMarketplace() =>
      _list(_source.getMarketplace);
  @override
  Future<Result<DemandForecast>> create(DemandForecast demand) async {
    try {
      return Success<DemandForecast>(await _source.create(_model(demand)));
    } catch (error) {
      return Failure<DemandForecast>('$error');
    }
  }
  ...
  Future<Result<DemandForecast>> updateMine(DemandForecast demand) async {
    try {
      return Success<DemandForecast>(await _source.updateMine(_model(demand)));
    } catch (error) {
      return Failure<DemandForecast>('$error');
    }
  }
