...
class SupabaseDemandRemoteDataSource implements DemandRemoteDataSource {
  ...
  Future<List<DemandForecastModel>> getMine() async {
    final rows = await _client.from(DatabaseTables.demandForecasts).select()
        .eq('buyer_id', _userId).order('needed_start_date');
  ...
  Future<DemandForecastModel> create(DemandForecastModel demand) async {
    final row = await _client.from(DatabaseTables.demandForecasts).insert({
      ...demand.toEditableJson(),
      'buyer_id': _userId,
      'commodity_id': demand.commodityId,
      'remaining_quantity': demand.quantity,
    }).select().single();
    await _client.rpc('run_demand_matching', params: {'p_demand_id': row['id']});
    return DemandForecastModel.fromJson(row);
  }

  @override
  Future<DemandForecastModel> updateMine(DemandForecastModel demand) async {
    final id = demand.id ?? (throw const BackendException('Demand ID wajib diisi.'));
    final row = await _client.from(DatabaseTables.demandForecasts)
        .update(demand.toEditableJson()).eq('id', id).eq('buyer_id', _userId)
        .select().single();
    return DemandForecastModel.fromJson(row);
  }
}
